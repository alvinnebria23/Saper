...
import { Column, Icon, IconButton, Button, Heading, Text, Row, ScrollView, Modal, Checkbox, Center, Image } from 'native-base';
...
const RegisterScreen = ({ navigation }) => {
  ...
  return (
      <ScrollView style={{ flex: 1, marginTop: '5%' }}>
          ...
          {render({control, onPressNext, onPressClearButton, errorInputFields, status, isConfirm, spinnerObject, onPressTutorial })}
          <>
            ...
            >
              <Modal.Content>
                <Modal.CloseButton />
                <Modal.Header>{modalType === "policy" ? "TERMS AND CONDITIONS | PRIVACY POLICY" : "APP ID AND SECRET KEY TUTORIAL"}</Modal.Header>
                <Modal.Body>
                <ScrollView showsVerticalScrollIndicator={false}>
                  {modalType === "policy"
                    ? applyBoldStyle(TERMS_AND_CONDITIONS_OBJECT)
                    : (
                      <Center>
                        <Text marginBottom={'5%'}>
                          1. Login to 
                          <Text
                            ...
                            onPress={() => Linking.openURL('https://affiliate.shopee.ph/open_api')}
                          >
                            {' https://affiliate.shopee.ph/open_api'}
                          </Text>
                          {'\n'}
                          2. Go to Open API
                          {'\n'}
                          3. Copy and paste the AppID & Secret
                        </Text>
                        <Image 
                          resizeMode='stretch'
                          width={'100%'}
...
const render = ({control, onPressNext, onPressClearButton, errorInputFields, status, isConfirm, spinnerObject, onPressTutorial }) => {
  return (
      <ProgressSteps {...PROGRESS_STEPS_STYLE}>
        ...
        >
          <Column space={2} paddingRight={'20%'} paddingLeft={'20%'} paddingBottom={'20%'}>
            ...
            <Center>
              <LinkButton 
                text={"How to get my App ID and Secret Key?"} 
                fontColor={"#0000FF"}
                fontSize={"sm"}
